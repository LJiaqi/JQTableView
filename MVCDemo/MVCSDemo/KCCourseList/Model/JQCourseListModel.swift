...
class JQCourseListModel: JQTableModel {
    let service = JQNetworkService()
    ...
    override func loadItems(_ parameters: [String : Any]?, completion: @escaping ([String : Any]?) -> Void, failure: @escaping (Error) -> Void) {
        ...
        service.request(parameters: nil, urlStr: urlStr1, completionHandler: { [weak self](response:JQNetResponse<ClassDetailResults>) in
    ...
    func wrapperItems(response:JQNetResponse<ClassDetailResults>){
        
        if let courseList = response.results?.courseSystem{
            
            var cellItems: [JQCourseListItem] = []
            for respose in courseList {
                let item = JQCourseListItem(with: respose)
                cellItems.append(item)
            }
            
            let sectionHeaderItem = JQCourseListSectionHeaderItem(withType: .header)
